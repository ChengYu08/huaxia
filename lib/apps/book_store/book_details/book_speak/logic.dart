...
class BookSpeakLogic extends GetxController {
  ...
  final ValueNotifier<Duration> streamDuration = ValueNotifier(0.minutes);

  var showBookText = false.obs;

  final ScrollController controller = ScrollController();
  var timers = false.obs;
  var volus = false.obs;
  var pace = false.obs;
  ...
  void onInit() {
    ...
    ttsApp.setCurrentCallback((int index, String speakText) {
      ...
      if (controller.position.haveDimensions) {
        double max =
            controller.position.maxScrollExtent / ttsApp.playList.length;
        controller.jumpTo(1);
        controller.animateTo(max * index,
  ...
  void onClose() {
    ...
    timers.value = false;
    volus.value = false;
    streamDuration.value=0.minutes;
  ...
  void goBook() {
    ...
    if (Get.currentRoute != Routers.bookReaderPage &&
        !Get.isRegistered<BookReaderLogic>(tag: '$bookId')) {
      Get.toNamed(Routers.bookReaderPage,
          arguments: bookChapters.value,
          parameters: {
            'bookId': '$bookId',
            'title': title,
            'author': author,
  ...
  play() async {
    ...
    if (speakText.bookParagraph != null) {
      List<String> textList = speakText
              .bookParagraph!.originalArticleElement.body?.children
              .map((e) => e.text)
              .toList() ??
          [];
  ...
  Future<BookParagraph?> addBookChapters(BookChapter bookChapter) async {
    ...
    try {
      final chapter = await Api.book_Chapters(
          bookId: bookId, chaptersId: bookChapter.paragraphId);
      if (chapter.success) {
        BookParagraph bookParagraph = BookParagraph(
            paragraphId: bookChapter.paragraphId,
