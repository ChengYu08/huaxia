...
class _TestState extends State<Test> {
  ...
  List< List<String>> getText({required String text, required double fontSize,
    ...
    while(tempText.isNotEmpty){
      List<String> lines = [];
        while(cutHeight<height){

          textPainter.text = TextSpan(text:tempText,style: TextStyle(fontSize: fontSize,height:lineHeight ));
          textPainter.layout(maxWidth: width);
          if((cutHeight+fontSize+lineHeight)>height || tempText.isEmpty){
            break;
          }
          int endTextIndex = textPainter.getPositionForOffset(Offset(width, 0)).offset;

          print("$endTextIndex==$tempText=");

          String  cutText= tempText.substring(0,endTextIndex);

          tempText = tempText.substring(endTextIndex ,tempText.length);
          lines.add(cutText);
            if(tempText.startsWith('\n')){
              tempText = tempText.substring(1,tempText.length);
            }
            cutHeight+=fontSize*lineHeight;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      floatingActionButton: FloatingActionButton(
        onPressed: (){
          data='';
          List.generate(3000, (index) => data+='正');
          setState(() {});},
      ),
      appBar: AppBar(
        title: Text('123'),
      ...
      body: Container(
        ...
        child: SizedBox(
          ...
          child:  LayoutBuilder(
            ...
            final v=  getText(text: data, fontSize: 16, width: constraints.maxWidth, height: constraints.maxHeight, lineHeight: 1.5);
              return PageView(
                children:v.map((e) => Text.rich(
                  TextSpan(
                    children: e.map((e1) => TextSpan(text: e1,style: TextStyle(fontSize: 16,height: 1.5))).toList(),
